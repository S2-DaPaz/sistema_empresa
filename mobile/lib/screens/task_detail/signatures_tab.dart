      ...
      children: [
        ...
        if (taskId != null) ...[
          ...
          AppDropdownField<String>(
            ...
            items: const [
              DropdownMenuItem(value: 'last_page', child: Text('Apenas ao final')),
              DropdownMenuItem(value: 'all_pages', child: Text('Todas as páginas')),
            ],
            onChanged: (value) => onSignatureScopeChanged(value ?? 'last_page'),
          ),
          ...
          if (signatureScope == 'all_pages' && signatureMode != 'none') ...[
            ...signaturePageItems.map((page) {
              ...
              final pageSignatures = signaturePages[key] as Map<String, dynamic>? ?? {};
              return Card(
                child: Padding(
                    ...
                    children: [
                      ...
                      if (signatureMode == 'client' || signatureMode == 'both')
                        SignaturePadField(
                          ...
                          onChanged: (value) => onUpdateSignaturePage(key, 'client', value),
                        ),
                      if (signatureMode == 'client' || signatureMode == 'both')
                        if ((pageSignatures['client']?.toString() ?? '').isNotEmpty)
                          Align(
                            alignment: Alignment.centerRight,
                            child: TextButton.icon(
                              onPressed: () => onUpdateSignaturePage(key, 'client', ''),
                              icon: const Icon(Icons.delete_outline),
                              label: const Text('Remover assinatura'),
                            ),
                          ),
                      ...
                      if (signatureMode == 'tech' || signatureMode == 'both')
                        SignaturePadField(
                          ...
                          onChanged: (value) => onUpdateSignaturePage(key, 'tech', value),
                        ),
                      if (signatureMode == 'tech' || signatureMode == 'both')
                        if ((pageSignatures['tech']?.toString() ?? '').isNotEmpty)
                          Align(
                            alignment: Alignment.centerRight,
                            child: TextButton.icon(
                              onPressed: () => onUpdateSignaturePage(key, 'tech', ''),
                              icon: const Icon(Icons.delete_outline),
                              label: const Text('Remover assinatura'),
                            ),
                          ),
          ...
          const SizedBox(height: 12),
          ElevatedButton(onPressed: onSave, child: const Text('Salvar assinaturas')),
        ],
      ],
    );
  }
